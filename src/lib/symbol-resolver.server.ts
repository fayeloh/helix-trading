...
export type ResolvedSymbol = {
  ...
  verified: boolean;
  source: string | null;
  failureReason?: "not_found" | "providers_unavailable";
};
...
export function normalizeSymbol(input: string, market: string): string {
  ...
  if (market === "CN")
    return /^[65]/.test(value) ? `${value}.SS` : `${value}.SZ`;
...
export async function resolveSymbol(
  ...
  const [quote, sec] = await Promise.all([
    ...
    inferredMarket === "US"
      ? getSecCompanyProfile(symbol).catch(() => null)
  ...
  const result: ResolvedSymbol = {
    ...
    type:
      quote?.instrumentType === "CRYPTOCURRENCY"
        ? "加密"
      ...
      sec && quote
        ...
        : sec
          ...
          : quote
            ? "Yahoo Finance"
            : null,
    ...(verified ? {} : { failureReason: "not_found" as const }),
  };
  try {
